...
class MIGC(nn.Module):
    ...
    def forward(self, ca_x, guidance_mask, other_info, return_fuser_info=False):
        ...
        dilated_mask = other_info['dilated_guidance_mask'] if 'dilated_guidance_mask' in other_info else None
        if dilated_mask is not None:
            dilated_mask = F.interpolate(dilated_mask, size=(H, W), mode='bilinear')
        ...
        if dilated_mask is not None:
            # print(ea_x.shape, dilated_mask.shape)
            ea_x = ea_x * dilated_mask.view(B, instance_num, HW, 1)
        else:
