...
import cv2
import math
from threeDIS.archs import MIGC, RefinedShader
...
class MIGCProcessor(nn.Module):
    def __init__(self, config, place_in_unet, migc_type='migc'):
        super().__init__()
        self.place_in_unet = place_in_unet
        self.not_use_migc = config['not_use_migc']
        self.naive_fuser = RefinedShader()        
        self.embedding = {}
    ...
    def __call__(
        ...
        if masks is None and sam_masks is None or not not_use_migc:
        ...
        if hidden_states.dtype == torch.float16:
            supplement_mask = supplement_mask.half()
            guidance_masks = guidance_masks.half()
            in_box = in_box.half()
        if not depth is None:
            in_depth = torch.from_numpy(np.array(depth[0]))[None, ...].float().to(cond_ca_output.device)  # (1, instance_num, 1)
        else:
            in_depth = None
        ...
        other_info['gamma_scale'] = gamma_scale
        other_info['sot'] = sot
...
class StableDiffusionL2DPipeline(StableDiffusionPipeline):
    def __init__(
            self,
            vae: AutoencoderKL,
