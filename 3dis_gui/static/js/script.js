...
function buildPrompt() {
  ...
  State.boxMap.forEach((box, box_id, map) => {
    ...
    prompt["Instance"+String(instance_num)] = nodeMIGCTextboxApply(
        ...
        box.x,
        box.y,
        document.getElementById("ca-input-" + box_id)? document.getElementById("ca-input-" + box_id).value: null,
        document.getElementById("ea-input-" + box_id)? document.getElementById("ea-input-" + box_id).value: null,
        document.getElementById("sac-input-" + box_id)? document.getElementById("sac-input-" + box_id).value: null
      );
    });

  prompt["InstanceNum"] = instance_num

  var editModeElement = document.getElementById("EditMode");
  if (editModeElement) {
    prompt["EditMode"] = document.getElementById("EditMode").checked
  }

  var UseSAMElement = document.getElementById("UseSAM");
  if (UseSAMElement) {
    prompt["UseSAM"] = document.getElementById("UseSAM").checked
  }
...
function handleImage(e) {
  ...
  reader.onload = function (event) {
    ...
    img.onload = function () {
      ...
      prompt = {}
      prompt["c_image"] = composeDepthImage().toDataURL("image/png");
      fetch('/set_depth_map', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
...
window.addEventListener("load", () => {
  ...
  let UseSAM = document.getElementById("UseSAM");
  if(UseSAM) {
    document.getElementById("UseSAM").checked = true;
  }


  let gamma = document.getElementById("gamma");
  if (gamma) {  // 如果元素存在
    State.gamma = State.gamma || 4;
    gamma.value = State.gamma;
    gamma.addEventListener("input", (event) => {
    State.gamma = event.target.value;
  });
  }
  

  let NaiveFuserSteps_input = document.getElementById("NaiveFuserSteps");
  if (NaiveFuserSteps_input) {
